...
theme = st.radio("Choose Theme:", ["Dark", "Light"], horizontal=True) plotly_theme = "plotly_dark" if theme == "Dark" else "plotly_white"

Assistant + Logo

st.image("https://svgshare.com/i/14fr.svg", width=120) st.image("https://github.com/user-attachments/assets/fe6dcdc0-bb78-42c7-a60f-f019fc910378", width=200) st.markdown("## 👋 Welcome to Slash Dynamics!") st.markdown("I'm your friendly assistant 🤖 here to help you analyze your call data.")

Upload section

uploaded_file = st.file_uploader("📄 Upload your call log file (.csv or .xlsx)", type=["csv", "xlsx"])

if uploaded_file: try: df = pd.read_csv(uploaded_file) if uploaded_file.name.endswith(".csv") else pd.read_excel(uploaded_file) except Exception as e: st.error(f"Error reading file: {e}") else: df.columns = df.columns.str.strip() df = df.dropna(axis=1, how='all')
...
else: st.info("\n✨ Please upload a call log file to begin your analysis!")
